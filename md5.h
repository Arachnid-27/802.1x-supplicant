...
void md5_update(struct md5_ctx* ctx, uint8_t* input, unsigned int input_len) {
    ...
    if (input_len >= part_len) {
        md5_memcpy((uint8_t*) &ctx->buffer[index], (uint8_t*) input, part_len);
    ...
    md5_memcpy((uint8_t*) &ctx->buffer[index], (uint8_t*) &input[i], input_len - i);
}

void md5_final(uint8_t digest[16], struct md5_ctx* ctx) {
    uint8_t bits[8];
    ...
    md5_memset((uint8_t*) ctx, 0, sizeof(struct md5_ctx));
}
